 ...
 */
package org.openhab.persistence.ibmbluemix.internal;
...
import org.openhab.io.transport.ibmbluemix.MqttService;
...
public class MqttPersistenceService implements PersistenceService {
    ...
    public void activate(final BundleContext bundleContext, final Map<String, Object> properties) {
        ...
        logger.debug("broker name : {} Topic Name : {} message Name : {} ", brokerName, topic, messageTemplate);
        logger.debug("Configuration updated for IBM bluemix Persistence.");
        ...
        logger.debug("Activating IBM bluemix Persistence");
    ...
    public void deactivate(final int reason) {
        ...
        logger.debug("Deactivating IBM bluemix Persistence");
    ...
    private String getProperty(Map<String, Object> properties, String name) {
        ...
        if (StringUtils.isNotBlank(value)) {
            ...
            logger.warn("mqtt-persistence1:" + name, "Missing or invalid property '" + name + "'");
            return null;
        }
    }
    ...
    public void store(Item item, String alias) {
        ...
        if (!configured) {
            logger.trace("IBM bluemix Persistence not configured yet. Cannot store item state for {}", item.getName());
            return;
        }
        ...
        try {
            ...
            logger.error("Error sending persistency message for item '{}' : {}", item.getName(), e.getMessage());
        }
    }
